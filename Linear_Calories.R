...
CalConWG <- calories_consumed

#Plot a Scatter Plot
plot(CalConWG$Calories.Consumed,CalConWG$Weight.gained..grams.)

#Other EDA and Plots
boxplot(CalConWG)

#Histogram
hist(CalConWG$Calories.Consumed)

hist(CalConWG$Weight.gained..grams.)

summary(CalConWG)

#Correation for weight gain and calories consumed
CC <-CalConWG$Calories.Consumed
WG <- CalConWG$Weight.gained..grams.
cor(WG,CC)
